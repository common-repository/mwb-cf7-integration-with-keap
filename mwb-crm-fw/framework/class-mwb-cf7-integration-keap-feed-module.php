<?php
/**
 * The complete management for the Keap-CF7 feeds custom post type.
 *
 * @since      1.0.0
 ...
 * @author     MakeWebBetter <https://makewebbetter.com>
 */
class Mwb_Cf7_Integration_Keap_Feed_Module {
	...
	/**
	 * Main Mwb_Cf7_Integration_Keap_Feed_Module Instance.
	 *
	 * Ensures only one instance of Mwb_Cf7_Integration_Keap_Feed_Module is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 *
	 * @static
	 * @return Mwb_Cf7_Integration_Keap_Feed_Module - Main instance.
	 */
	...
	public function register_feeds_post_type() {
		...
		$labels = array(
			'name'               => _x( 'Feeds', 'Post Type General Name', 'mwb-cf7-integration-with-keap' ),
			'singular_name'      => _x( 'Feed', 'Post Type Singular Name', 'mwb-cf7-integration-with-keap' ),
			'menu_name'          => _x( 'Crm Feeds', 'Admin menu name', 'mwb-cf7-integration-with-keap' ),
			'parent_item_colon'  => __( 'Parent Feed', 'mwb-cf7-integration-with-keap' ),
			'all_items'          => __( 'All Feeds', 'mwb-cf7-integration-with-keap' ),
			'view_item'          => __( 'View Feed', 'mwb-cf7-integration-with-keap' ),
			'add_new_item'       => __( 'Add New Feed', 'mwb-cf7-integration-with-keap' ),
			'add_new'            => __( 'Add New', 'mwb-cf7-integration-with-keap' ),
			'edit_item'          => __( 'Edit Feed', 'mwb-cf7-integration-with-keap' ),
			'update_item'        => __( 'Update Feed', 'mwb-cf7-integration-with-keap' ),
			'search_items'       => __( 'Search Feed', 'mwb-cf7-integration-with-keap' ),
			'not_found'          => __( 'Not Found', 'mwb-cf7-integration-with-keap' ),
			'not_found_in_trash' => __( 'Not found in Trash', 'mwb-cf7-integration-with-keap' ),
		);

		$args = array(
			'label'                => __( 'Feeds', 'mwb-cf7-integration-with-keap' ),
			'description'          => __( 'Feeds for crm', 'mwb-cf7-integration-with-keap' ),
			'labels'               => $labels,
	...
	public function add_feed_meta_box() {
		...
		$metaboxes[] = array(
			...
			'title'    => esc_html__( 'Feed details', 'mwb-cf7-integration-with-keap' ),
		...
		$metaboxes[] = array(
			...
			'title'    => esc_html__( 'Conditional Statements', 'mwb-cf7-integration-with-keap' ),
	...
	public function get_pro_callback( $key = '' ) {
		...
		switch ( $key ) {
			...
			case 'pro-manual-sync':
				$result = array( $pro_class, 'mwb_cf7_int_pro_feeds_manual_sync' );
				break;

			case 'pro-add-notes':
				$result = array( $pro_class, 'mwb_cf7_int_pro_feeds_add_notes' );
				break;

			case 'pro-associate-contact':
				$result = array( $pro_class, 'mwb_cf7_int_pro_feeds_associate_contact' );
				break;

			case 'pro-associate-company':
				$result = array( $pro_class, 'mwb_cf7_int_pro_feeds_associate_company' );
				break;

			case 'pro-add-attahcment':
				$result = array( $pro_class, 'mwb_cf7_int_pro_feeds_add_attachments' );
				break;

			case 'pro-add-owner':
				$result = array( $pro_class, 'mwb_cf7_int_pro_feeds_add_owner' );
				break;
			...
			case 'pro-add-campaign':
				$result = array( $pro_class, 'mwb_cf7_int_pro_feeds_associate_campaign' );
				break;

			case 'pro-add-lead':
				$result = array( $pro_class, 'mwb_cf7_int_pro_feeds_add_lead' );
				break;

			case 'pro-add-pipeline':
				$result = array( $pro_class, 'mwb_cf7_int_pro_feeds_add_pipeline' );
				break;
