 ...
 * @subpackage Mwb_Cf7_Integration_With_Keap/public
 */
 ...
 */
class Mwb_Cf7_Integration_With_Keap_Public {
	...
	/**
	 * Main Mwb_Cf7_Integration_With_Keap_Public Instance.
	 *
	 * Ensures only one instance of Mwb_Cf7_Integration_With_Keap_Public is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 *
	 * @static
	 * @return Mwb_Cf7_Integration_With_Keap_Public - Main instance.
	 */
	public static function get_instance() {

		if ( null == self::$_instance ) { //phpcs:ignore

			self::$_instance = new self( 'mwb-cf7-integration-with-keap', MWB_CF7_INTEGRATION_WITH_KEAP_VERSION );
		}
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/mwb-cf7-integration-with-keap-public.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/mwb-cf7-integration-with-keap-public.js', array( 'jquery' ), $this->version, false );
			...
			foreach ( $active_feeds as $key => $feed_id ) {
				...
				if ( ! empty( $filter_exist ) ) {
					...
					if ( true === $filter_result ) { // If filter results true, then send data to CRM.
						...
						$result = $api_module->create_or_update_record( $crm_object, $request, $log_data );
		...
		if ( isset( $response['success'] ) && false == $response['success'] ) { // phpcs:ignore
			if ( null !== $this->connect_manager->get_settings_details( 'notif' ) ) {
				if ( isset( $response['response'] ) ) {
					$this->mwb_cf7_integration_send_email( $response['response'], $data );
				}
			}
	...
	public function mwb_cf7_integration_send_email( $info, $data ) {
		...
		if ( ! empty( $info ) && is_array( $info ) ) {
			...
			$subject   = sprintf( esc_html__( 'Error While Posting CF7 form data over %s ', 'mwb-cf7-integration-with-keap' ), esc_html( $this->crm_name ) );
			...
			if ( isset( $info['data'] ) && isset( $info['data'] ) ) {
				...
				$info_detail = is_array( $info ) ? wp_json_encode( $info ) : $info;
			}

			$email_data = array(
				'Title'          => esc_html__( 'MWB CF7 Integration with Keap', 'mwb-cf7-integration-with-keap' ),
